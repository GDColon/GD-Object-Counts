...
let totalObjects = 1920
let objects = {}
let unique = {}
let objTable = Array(totalObjects + 1).fill().map(() => Array(lvls.length + 1).fill(0)) // 2D table of zeroes
...
lvls.forEach((file, index) => {
    ...
    Object.keys(obj).forEach(objID => {
        ...
        objTable[objID == "total" ? 1 : (+objID) + 1][index+1] = count // add to table
    })
    let [levelID, levelName] = file.split(".json")[0].split(" - ") // parse name and ID from filename
    objTable[0][index+1] = `"${levelName} (${levelID})"` // name on row 1
})
